...
(defun foo (c discriminator prev-score-map b)
  (let* ((score-map (make-empty-matrix (board-dimensions b)))
    ...
    (mapc (lambda (pos)
            (let ((neighbors (non-empty-neighbors pos prev-score-map)))
              (let ((origins (mapcar (lambda (pos) (char-at pos prev-score-map)) neighbors)))
                (let ((flat-origins (apply #'append origins)))
                  (set-char-at (funcall discriminator flat-origins) pos score-map)))))
          score-pos)
    score-map))

(defun sum-of-trailhead-scores (discriminator file)
  (let ((b (read-character-matrix-file file)))
    (let* ((score-map (make-empty-matrix (board-dimensions b)))
      ...
      (mapc (lambda (c) (setf score-map (foo c discriminator score-map b)))
            (list #\8 #\7 #\6 #\5 #\4 #\3 #\2 #\1 #\0))
      (reduce #'+ (mapcar (lambda (pos) (length (char-at pos score-map))) (find-char #'identity score-map))))))

(assert (= (sum-of-trailhead-scores #'remove-duplicates "test.txt") 36))
(print (sum-of-trailhead-scores #'remove-duplicates "input.txt"))
(assert (= (sum-of-trailhead-scores #'identity "test.txt") 81))
(print (sum-of-trailhead-scores #'identity "input.txt"))
