...
sub get($x,$y) {
    return '.' if($x<0 or $x>=$num_cols or $y<0 or $y>=$num_rows);
    return substr($rows[$y],$x,1);
}

sub get_num($s, $x,$y) {
    my $c = get($x,$y);
    return $s unless (isdigit($c));
    return get_num($s.$c,$x+1,$y);
}
sub isdigit($a) {
    return $a=~/[0-9]/ ? 1 : 0;
}
...
sub issymbol($a) {
    #return ($a ne ".") and (not isdigit($a));
    return 0 if ($a eq ".");
    return 0 if (isdigit($a));
    return 1;
}
...
sub symbol_near($x, $y, $len) {
    my $n=0;
    $star_x=-1;
    $star_y=-1;
    for(my $xx=$x-1;$xx<($x+$len+1);$xx++) {
	for(my $yy=$y-1;$yy<=($y+1);$yy++) {
	    my $c=get($xx,$yy);
	    if(issymbol($c)) {
		#print "Found symbol $c at $xx,$yy\n";
		$n=1;
		if($c eq '*') {
		    die "Multiple stars ($star_x,$star_y) and ($xx,$yy)" if($star_x!=-1);
		    $star_x=$xx;
		    $star_y=$yy;
		}
	    }
...
while(<DATA>) {
	...
	for(my $y=0;$y<$num_rows;$y++) {
		...
		my $c=get($x,$y);
		my $c_before=get($x-1,$y);
		next unless(isdigit($c) and not isdigit($c_before));
		my $num=get_num("", $x,$y);
		if (symbol_near($x,$y,length($num))) {
		    #print(" +$num\n");
		    #$sum+=int($num);

		    if($star_x!=-1) {
			my $k="$star_x$star_y";
			if(exists($star_hash{$k})) {
			    print("$num*$star_hash{$k}");
			    $sum+=int($num)*int($star_hash{$k});
			    #print("\t => $sum");
			    delete($star_hash{$k});
			    print "\n";
			}
