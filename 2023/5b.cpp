...
bool operator<(Range const &a, Range const &b) { return a.start < b.start; }
    ...
    while (true) {
        ...
        for (int i = 0; i < m.size(); i += 3) {
            ...
            for (auto input_elem : input) {
                auto [new_mapped, new_input_elems] =
                    inside_and_outside(input_elem, mr);
                ...
                copy(new_input_elems.begin(), new_input_elems.end(),
                     back_inserter(new_input));
                copy(new_mapped.begin(), new_mapped.end(),
